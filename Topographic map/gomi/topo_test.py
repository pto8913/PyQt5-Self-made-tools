...
class Main(QWidget):
  ...
  #def searchArea(self):
    ...
    #area = {t: result[0][t] for t in result[0]}
    #self.Area.setText(area["admin1"]+" "+area["name"])
  ...
  def selectFile(self):
    try:
      ...
      # ファイルが存在するディレクトリだよ
      self.OpenFileRoot = root + self.path +  "/_json_n_txt/txt/"
      print(self.OpenFileRoot + self.FileName)
      # ファイルを開いて緯度経度を受け取るよ
  ...
  def setDirList(self):
    ...
    for _, dirs, _ in os.walk(root):
      for d in dirs:
        if "json" not in d and "txt" not in d:
