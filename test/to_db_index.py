...
from PyQt5.QtWidgets import (
  ...
  QGridLayout, QVBoxLayout, QLineEdit, QInputDialog, QFileDialog)
...
class Main(QWidget):
  ...
  def clickedStart(self):
    ...
    for (index, xml_path) in enumerate(self.xmlPathList):
      ...
      conn = sqlite3.connect(self.db_dir + dbname + ".db")
      cur = conn.cursor()
      sql = cur.execute
  ...
  def inputDBName(self):
    dbname, ok = QInputDialog.getText(self, "Enter Database Name", "Database Name: ", QLineEdit.Normal, "")
    if not ok:
      return False
    if dbname:
      path = self.db_dir + dbname
      if os.path.exists(path):
        QMessageBox.information(self, "name Error", "This database is already exists <br> Please change.", QMessageBox.Ok)
        return self.inputDBName()
      return dbname
  ...
  def clickedAdd(self):
    filename, ok = QFileDialog.getOpenFileName(self, "Open File", self.xml_dir, filter = "xml file (*.xml)")
    # if clicked cancel
    if not ok:
      return 
    self.FileList.addItem(os.path.basename(filename))
